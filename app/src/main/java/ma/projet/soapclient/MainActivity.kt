...
import ma.projet.soapclient.adapters.CompteAdapter
...
class MainActivity : AppCompatActivity() {
    ...
    /**
     * Affiche la boîte de dialogue pour modifier un compte existant.
     */
    private fun showEditCompteDialog(compte: Compte) {
        ...
        MaterialAlertDialogBuilder(this)
            ...
            .setPositiveButton("Modifier") { _, _ ->
                val solde = etSolde.text.toString().toDoubleOrNull() ?: compte.solde
                val type = if (radioCourant.isChecked) TypeCompte.COURANT else TypeCompte.EPARGNE

                lifecycleScope.launch(Dispatchers.IO) {
                    val success = service.updateCompte(compte.id!!, solde, type)
                    withContext(Dispatchers.Main) {
                        if (success) {
                            adapter.replaceCompte(compte.copy(solde = solde, type = type))
                            Toast.makeText(this@MainActivity, "Compte modifié.", Toast.LENGTH_SHORT).show()
                        } else {
                            Toast.makeText(this@MainActivity, "Erreur lors de la modification.", Toast.LENGTH_SHORT).show()
                        }
                    }
                }
            }
